// 002
// 001 is based on integer,
// but in this project based on probability 0 to 1.

// 20190809 i start to rewrite int to float. now not done yet.
...
void ofApp::draw(){
	...
	for (i = 1; i < (Width / xZoom); i++) {
		for (j = 1; j < 50; j++) {
			if (abs(buffer[i] - buffer[i - j])/j < 0.003) {
